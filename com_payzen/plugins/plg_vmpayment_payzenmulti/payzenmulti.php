 ...
 * This file is part of Lyra Collect plugin for VirtueMart. See COPYING.md for license details.
 *
...
class plgVMPaymentPayzenMulti extends vmPSPlugin
{
    // Instance of class.
    ...
    function __construct(& $subject, $config)
    {
        ...
        if ($plugin_features['qualif']) {
            $varsToPush['ctx_mode']['0'] = 'PRODUCTION';
        }
    ...
    function getTableSQLFields()
    {
        $SQLfields = array(
            ...
            'tax_id'                                          => 'smallint(1)',
            'payzenmulti_custom'                                => 'varchar(255)',
            'payzenmulti_response_payment_amount'               => 'char(15)',
            'payzenmulti_response_auth_number'                  => 'char(10)',
            'payzenmulti_response_payment_currency'             => 'char(3)',
            'payzenmulti_response_payment_mean'                 => 'char(255)',
            'payzenmulti_response_payment_date'                 => 'char(20)',
            'payzenmulti_response_payment_status'               => 'char(3)',
            'payzenmulti_response_payment_message'              => 'char(255)',
            'payzenmulti_response_payment_card_brand_choice'    => 'char(255)',
            'payzenmulti_response_card_number'                  => 'char(50)',
            'payzenmulti_response_trans_id'                     => 'char(50)',
            'payzenmulti_response_expiry_month'                 => 'char(2)',
            'payzenmulti_response_expiry_year'                  => 'char(4)'
        );
    ...
    /**
     * Prepare data and redirect to Lyra Collect payment platform
     *
    ...
    function plgVmConfirmedOrder($cart, $order)
    {
        ...
        if ($currency == null) {
            $this->logInfo('plgVmOnConfirmedOrderGetPaymentForm -- Could not find currency numeric code for currency : ' . $currency_obj->currency_numeric_code, 'error');
        ...
        $version = JVERSION . '_' . vmVersion::$RELEASE;
        $request->set('contrib', com_payzenInstallerScript::getDefault('CMS_IDENTIFIER') . '_' . com_payzenInstallerScript::getDefault('PLUGIN_VERSION') . '/' . $version . '/' . PHP_VERSION);
        ...
        $request->set('cust_address', $usrBT->address_1 . ' ' . $usrBT->address_2);
        ...
        $form .= '<br />';
        $form .= '<input type="image" name="submit" src="' . JURI::base(true) . '/images/stories/virtuemart/payment/' . 'payzenmulti.png' . '" alt="' . JText::_('VMPAYMENT_' . $this->_name . '_BTN_ALT') . '" title="' . JText::_('VMPAYMENT_PAYZEN_BTN_ALT') . '"/>';
        ...
        die(); // Do not save order, not send mail, do redirect.
    }

    /**
     * Check Lyra Collect response, save order if not done by server call and redirect to response page
    ...
    function plgVmOnPaymentResponseReceived(&$html)
    {
        ...
        if (! $payzen_response->isAuthentified()) {
            $this->logInfo('plgVmOnPaymentResponseReceived -- ' . $_SERVER['REMOTE_ADDR'] . ' sent suspect request without valid signature with parameters: ' . print_r($data, true), 'error');
            $this->logInfo('plgVmOnPaymentResponseReceived -- Signature algorithm selected in module settings must be the same as one selected in Lyra Expert Back Office.', 'error');
        ...
        if (($method->ctx_mode === 'TEST') && $plugin_features['prodfaq']) {
            // TEST mode : notification URL is correctly configured, just show pass to prod info.
        ...
        if ($order_status_code == 'P') {
            $this->logInfo('plgVmOnPaymentResponseReceived -- notification URL does not work.', 'warning');

            if ($method->ctx_mode === 'TEST') {
                if (JFactory::getConfig()->get('offline') == 1 || VmConfig::get('shop_is_offline') == '1') {
                    // TEST mode warning : shop is in maintenance mode.
                    $msg = '<div style="margin-bottom: 15px;">' . JText::_('VMPAYMENT_' . $this->_name . '_MAINTENANCE_MODE_WARN'). '</div>';
                } else {
                    // TEST mode warning : notification URL not correctly called.
    ...
    /**
     * Process a Lyra Collect payment cancellation.
     *
     * @param int $virtuemart_order_id virtuemart order primary key concerned by payment
     * @return
     */
    ...
    /**
     * Check Lyra Collect response, save order and empty cart(if payment success) when server notification is received from payment platform.
     *
    ...
    function plgVmOnShowOrderBEPayment($virtuemart_order_id, $payment_method_id)
    {
        ...
        $html .= $this->getHtmlRowBE($this->_name . '_TRANS_UUID', $payment_data->$response_trans_id ? $payment_data->$response_trans_id : '-');
    ...
    function savePaymentData($virtuemart_order_id, $payzen_response)
    {
        ...
        $response[$this->_name . '_response_payment_amount'] = $payzen_response->get('amount');
        ...
        $response[$this->_name . '_response_payment_status'] = $payzen_response->get('code');
        ...
        $response[$this->_name . '_response_trans_id'] = $payzen_response->get('trans_uuid');
    ...
    function managePaymentResponse($virtuemart_order_id, $payzen_response, $new_status, $session_id = null, $order_number=null)
    {
