...
class plgVMPaymentPayzen extends vmPSPlugin
{
    ...
    function getTableSQLFields()
    {
        $SQLfields = array(
            ...
            'payzen_response_payment_card_brand_choice'    => 'char(255)',
            'payzen_response_card_number'                  => 'char(50)',
            'payzen_response_trans_id'                     => 'char(50)',
    ...
    function plgVmConfirmedOrder($cart, $order)
    {
        ...
        $form .= '<br />';
        $form .= '<input type="image" name="submit" src="' . JURI::base(true) . '/images/stories/virtuemart/payment/' . 'payzen.png' . '" alt="' . JText::_('VMPAYMENT_' . $this->_name . '_BTN_ALT') . '" title="' . JText::_('VMPAYMENT_PAYZEN_BTN_ALT') . '"/>';
    ...
    /**
     * Check PayZen response, save order if not done by notification call and redirect to response page
    ...
    function plgVmOnPaymentResponseReceived(&$html)
    {
        ...
        if (! $payzen_response->isAuthentified()) {
            ...
            $this->logInfo('plgVmOnPaymentResponseReceived -- Signature algorithm selected in module settings must be the same as one selected in PayZen Back Office.', 'error');
    ...
    function plgVmOnShowOrderBEPayment($virtuemart_order_id, $payment_method_id)
    {
        ...
        $html .= $this->getHtmlRowBE($this->_name . '_PAYMENT_NAME', $payment_data->payment_name);
